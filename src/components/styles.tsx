  ...
  return (
    <DraggableModal
      ...
      headerLeft={stylePage && (
        <button className="icon-btn" onClick={() => { const shouldExecute = (window as any).juicecut?.settings?.executeHeaderButtonsOnDrag ?? true; if (shouldExecute || !(window as any).__backButtonDragged) { const parentId = parentMap[stylePage]; setStylePage(parentId || null); } }} onMouseDown={() => { (window as any).__backButtonDragged = false; }} onMouseMove={(e) => { if ((window as any).__backButtonDragged) return; const dx = Math.abs(e.clientX - (e as any).clientX); const dy = Math.abs(e.clientY - (e as any).clientY); if (dx > 3 || dy > 3) (window as any).__backButtonDragged = true; }} style={{ position: 'absolute', left: 8, top: '50%', transform: 'translateY(-50%)', width: 26, height: 26, color: 'var(--text-secondary)' }} title="Back">
